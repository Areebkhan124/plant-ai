...
# Disable XLA JIT before importing/using TensorFlow. On CPUs without AVX-512,
# XLA's compilation of this model's mixed-precision (float16) ops falls back
# to an expensive emulation path that spikes memory usage enough to get the
# process OOM-killed on memory-constrained hosts like Railway.
...
# Force the whole model to run in float32 (instead of mixed_float16) so no
# float16 emulation/XLA compilation is needed on CPU.
...
# CONSTANTS
# ─────────────────────────────────────────────────────────────
MODEL_PATH     = "convnext_plant_disease.keras"   # local filename fallback
CLASS_CSV_PATH = "class_dict.csv"
IMG_SIZE       = (224, 224)

# The model file is 446MB, way over GitHub's 100MB limit, so it isn't committed
# to git at all. Instead it's downloaded from a Hugging Face Hub MODEL repo at
# startup. Set these via Railway's environment variables (Step 2 in the guide):
#   HF_MODEL_REPO_ID  -> e.g. "Areebkhan124/planta-plant-disease-model"
#   HF_MODEL_FILENAME -> e.g. "convnext_plant_disease.keras"
#   HF_TOKEN          -> only needed if the model repo is private
HF_MODEL_REPO_ID  = os.getenv("HF_MODEL_REPO_ID", "")
HF_MODEL_FILENAME = os.getenv("HF_MODEL_FILENAME", MODEL_PATH)
HF_MODEL_REPO_TYPE = os.getenv("HF_MODEL_REPO_TYPE", "model")  # "model" or "space"
HF_TOKEN          = os.getenv("HF_TOKEN")  # optional, only for private repos


def resolve_model_path() -> str:
    """
    Returns a local filesystem path to the .keras model file.
    Priority:
      1. If a valid local file already exists (e.g. for local testing), use it.
      2. Otherwise, download it from the Hugging Face Hub model repo.
    """
    ...
    if not HF_MODEL_REPO_ID:
        raise RuntimeError(
            "Model file not found locally and HF_MODEL_REPO_ID is not set. "
            "Set the HF_MODEL_REPO_ID environment variable on Railway to "
            "download the model from the Hugging Face Hub."
        )

    print(f"[INFO] Downloading model from Hugging Face Hub: {HF_MODEL_REPO_ID}/{HF_MODEL_FILENAME} (repo_type={HF_MODEL_REPO_TYPE})")
    ...
    print(f"[INFO] Model downloaded to: {downloaded_path}")
...
try:
    ...
    print(f"[INFO] SUCCESS: Model loaded natively from {resolved_path}")
except Exception as exc:
    print(f"[ERROR] Native load failed: {exc}")
    try:
        resolved_path = resolve_model_path()
        model = tf.keras.models.load_model(resolved_path, compile=False, safe_mode=False)
...
# Build the Grad-CAM sub-model ONCE here at startup (not per-request). Rebuilding
# this on every request was causing repeated tf.function retracing, which spikes
# memory usage and was causing the process to be OOM-killed under load on Railway.
grad_model: tf.keras.Model = None  # type: ignore
if model is not None:
    try:
        _gradcam_layer = _find_gradcam_layer(model)
        grad_model = tf.keras.models.Model(
            inputs=model.inputs,
            outputs=[_gradcam_layer.output, model.output],
        )
        print(f"[INFO] Grad-CAM sub-model built once at startup (layer: {_gradcam_layer.name}).")
    except Exception as exc:
        print(f"[WARNING] Could not pre-build Grad-CAM sub-model: {exc}")
...
def compute_gradcam(pil_img: Image.Image, preprocessed: np.ndarray, pred_class_idx: int) -> str:
    ...
    try:
        if grad_model is None:
            raise RuntimeError("Grad-CAM sub-model was not built at startup.")
